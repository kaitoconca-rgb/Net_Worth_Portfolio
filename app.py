...
with st.spinner("Ripristino totale in corso..."):
    prices_now = []
    cache_prezzi = {}
...
with tab2:
    ...
    df_raw['% Vendi'] = 0.0
    # RIPRISTINATO: Prezzo_Acq incluso per calcolo tasse
    ...
    if ed_df['% Vendi'].sum() > 0:
        ...
        st.success(f"Gain Lordo: ${sim['G_Sim'].sum():,.2f} AUD | Imponibile: ${max(0, sim['Taxable'].sum()):,.2f} AUD")

with tab3:
    st.subheader("Evoluzione Storica")
    # RIPRISTINATO: Logica di accumulo cronologico corretto
    h_df = df_raw.sort_values('Date_DT').copy()
    h_df['Investito_Cum'] = h_df['Inv_EUR'].cumsum()
    # Per il valore di mercato storico, usiamo l'accumulo delle quote valorizzate al prezzo attuale (approssimazione)
    ...
    fig_h.add_trace(go.Scatter(x=h_df['Date_DT'], y=h_df['Valore_Cum'], name="Valore Corrente", fill='tonexty', line_color='#1f77b4'))
    st.plotly_chart(fig_h, use_container_width=True)

with tab4:
    st.subheader("🛠️ Monitoraggio Prezzi")
    diag_list = []
    for k, v in ticker_diag.items():
        p_val = cache_prezzi.get(k)
        p_str = f"{p_val:.2f} €" if p_val is not None else "N/D"
        diag_list.append({"ISIN": k, "Status": v["status"], "Delay": v["delay"], "Last Price": p_str})
